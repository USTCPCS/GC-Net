...
from NNetWork import GCNet
from Dataset import Kitty2015DataSet, SceneFlow
from Loss import RegLoss, Validation
...
class Model(object):
    ...
    def build_net(self):
        ...
        self.dataset['val'] = Kitty2015DataSet(root_dir=config['root_dir'],is_validation=True)
            ...
            for batch_index in range(config['batches_per_epoch']):
                ...
                if (batch_index + 1) % config['batches_per_validation'] == 0:
                    ...
                    for i in range(num):
                        ...
                        v1_t += v1
                        v2_t += v2
                    print('.....>2 px : {0}%  >3 px : {1}%  >5 px : {2}%'.format(v0_t / num, v1_t / num, v2_t / num))
    ...
    def predict(self):
        for i in range(10):
            ...
            # disp_prediction = Image.fromarray(disp_prediction)
            # disp_prediction.show()
            # disp_gt = Image.fromarray(disp_gt)
            # disp_gt.show()
            # disp_gt.save(os.path.join(dir, 'disp_gt.png'))
...
if __name__ == '__main__':
    ...
    model.train()
    # model.predict()
    # for i, par in enumerate(model.net.parameters()):
    #     print(i, par.size())
